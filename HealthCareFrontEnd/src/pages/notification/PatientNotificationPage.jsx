...
function PatientNotificationPage() {
  ...
  const patientCode = "PAT001"; // The patient code to filter notifications by (you can also make this dynamic)

  useEffect(() => {
    // Fetch all notifications from the backend using notification service
    viewNotification()
      .then(response => {
        // Filter notifications by exact patientCode match
        const filteredNotifications = response.data.filter(notification => notification.patientCode === patientCode);
        setNotifications(filteredNotifications);
      })
      .catch(error => console.error('Error fetching notifications:', error));
  }, [patientCode]); // Dependency on patientCode so it refetches if it changes
